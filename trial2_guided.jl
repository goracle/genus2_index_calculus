...
# ---------------------------------------------------------------------------
#  Guided anchor selection to discourage rapid expander formation.
# ---------------------------------------------------------------------------
...
function update_guidance!(G::WalkGuidance, row::Dict{Int,Int})
    # Only the support matters here: count each column once per relation.
    for (col, v) in row
        v == 0 && continue
...
function anchor_score(pt::NTuple{2,Int}, pt2idx::Dict{NTuple{2,Int},Int},
                      guidance::WalkGuidance; current::Union{Nothing,NTuple{2,Int}}=nothing)
    idx = get(pt2idx, pt, 0)
    idx == 0 && return -Inf

    deg   = get(guidance.col_degree, idx, 0)
    rec   = count(==(idx), guidance.recent_cols)

    score = -3.0 * deg - 0.75 * rec

    # Mildly discourage immediate backtracking.
    if current !== nothing && pt == current
        score -= 1.5
    end

    # Mild randomness avoids deterministic ties that can overfit the first atom.
    score += 1e-6 * rand()
    return score
end

function choose_anchor(cands::Vector{NTuple{2,Int}},
                       pt2idx::Dict{NTuple{2,Int},Int},
                       guidance::WalkGuidance;
                       current::Union{Nothing,NTuple{2,Int}}=nothing,
                       guided::Bool=true)
    isempty(cands) && error("choose_anchor called with no candidates")
    if !guided || length(cands) == 1
        return cands[rand(1:length(cands))]
    end

    best_pt = cands[1]
    best_sc = -Inf
    for pt in cands
        sc = anchor_score(pt, pt2idx, guidance; current=current)
        if sc > best_sc
            best_sc = sc
            best_pt = pt
        end
...
function choose_fb_seed(fb::Vector{NTuple{2,Int}},
                        pt2idx::Dict{NTuple{2,Int},Int},
                        guidance::WalkGuidance;
                        guided::Bool=true)
    isempty(fb) && error("choose_fb_seed called with empty factor base")
    return choose_anchor(fb, pt2idx, guidance; current=nothing, guided=guided)
end
...
function index_calculus_walk(G::Div2, T::Div2;
    ...
    for step in 1:walk_steps
        ...
        if !fb_frozen
            ...
            # Advance in chain; steer to a low-degree anchor when guided.
            cur_pt = choose_anchor([P0, R, S], pt2idx, guidance; current=P0, guided=guided)
            if length(fb) >= fb_size
                fb_frozen = true
                cur_pt = choose_fb_seed(fb, pt2idx, guidance; guided=guided)
        ...
        # Advance cur_pt: stay in FB if possible, but prefer low-degree anchors.
        cands = NTuple{2,Int}[]
        push!(cands, P0)
        ir != 0  && push!(cands, R)
        is_ != 0 && push!(cands, S)
        cur_pt = choose_anchor(cands, pt2idx, guidance; current=P0, guided=guided)
