...
# ---------------------------------------------------------------------------
@inline function _lsm_record_sample!(sc::LP1ConjLSMStore, fp::UInt64, now_t::Float64, key::CanonicalLP1Key)
...
# ---------------------------------------------------------------------------
@inline function _bday_record_collision!(sc::LP1ConjLSMStore, now_t::Float64)
    lock(sc.bday_lock)
    try
...
# ---------------------------------------------------------------------------
function lsm_bday_report(sc::LP1ConjLSMStore, p::Integer, r::Real; io::IO = stdout)
    # ── Aggregate across all peer LSMs ─────────────────────────────────────
    # Each thread has its own LP1ConjLSMStore instance linked via `peers`.
