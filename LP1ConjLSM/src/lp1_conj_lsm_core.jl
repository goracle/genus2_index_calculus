# =============================================================================
#  lp1_conj_lsm_core.jl — LP1ConjLSMStore struct, hot-shard layer, public API
#
...
# ---------------------------------------------------------------------------
#  LP1ConjLSMStore — the main struct
# ---------------------------------------------------------------------------
mutable struct LP1ConjLSMStore{V}
    # Hot RAM table
    ...
    peers       ::Vector{Any}   # Vector{LP1ConjLSMStore{V}} — typed as Any to avoid forward-ref
...
# ---------------------------------------------------------------------------
function _lsm_open_read_io!(sc::LP1ConjLSMStore)
...
# passing sc's fields directly so the disk module stays LP1ConjLSMStore-agnostic.
@inline function _sc_pread_record!(sc::LP1ConjLSMStore, buf::Vector{UInt8}, off::Int)
    _pread_record!(sc.spill_read_io::Cint, buf, off)
end

@inline function _sc_fadvise_dontneed!(sc::LP1ConjLSMStore)
...
@inline function _lsm_hot_find(sc::LP1ConjLSMStore, si::Int,
...
@inline function _lsm_hot_insert!(sc::LP1ConjLSMStore{V}, si::Int,
...
@inline function _lsm_hot_delete!(sc::LP1ConjLSMStore, si::Int, slot::Int)
    keys = sc.hot_keys[si]
    vals = sc.hot_vals[si]
    cap  = sc.hot_caps[si]
    mask = sc.hot_masks[si]
...
# ---------------------------------------------------------------------------
function _lsm_flush_shard!(sc::LP1ConjLSMStore{V}, si::Int) where V
    n = sc.hot_counts[si]
    n == 0 && return
...
# ---------------------------------------------------------------------------
@inline function _sc_snapshot_read_fd(sc::LP1ConjLSMStore)
    lock(sc.file_lock)
    runs_snap = sc.runs
...
@inline function _sc_try_snapshot_read_fd(sc::LP1ConjLSMStore)
...
@inline function _sc_release_read_fd!(sc::LP1ConjLSMStore)
    Threads.atomic_sub!(sc.spill_read_refcount, 1)
    nothing
end

function _sc_spill_read_drain_and_close!(sc::LP1ConjLSMStore)
...
# ---------------------------------------------------------------------------
function _lsm_compact!(sc::LP1ConjLSMStore)
...
# ---------------------------------------------------------------------------
function _sc_disk_find(sc::LP1ConjLSMStore,
                        key::CanonicalLP1Key,
...
function _sc_disk_delete!(sc::LP1ConjLSMStore, ri::Int, pos::Int)
...
function conj_total_entries(sc::LP1ConjLSMStore)::Int
    sum(sc.hot_counts) + sc.n_disk_live
end

function Base.haskey(sc::LP1ConjLSMStore, key::CanonicalLP1Key)::Bool
    conj_haskey(sc, conj_shard_idx(key), key)
end

function Base.getindex(sc::LP1ConjLSMStore{V}, key::CanonicalLP1Key)::V where V
    conj_getval(sc, conj_shard_idx(key), key)
end

function conj_haskey(sc::LP1ConjLSMStore, si::Int, key::CanonicalLP1Key)::Bool
...
function conj_getval(sc::LP1ConjLSMStore{V}, si::Int, key::CanonicalLP1Key)::V where V
    lock(sc.shard_locks[si])
    try
...
function conj_pop!(sc::LP1ConjLSMStore{V}, si::Int, key::CanonicalLP1Key)::V where V
    lock(sc.shard_locks[si])
    try
        slot = _lsm_hot_find(sc, si, key)
        if slot != 0
...
function conj_pop_safe(sc::LP1ConjLSMStore{V}, si::Int, key::CanonicalLP1Key)::Union{V,Nothing} where V
    lock(sc.shard_locks[si])
    try
        slot = _lsm_hot_find(sc, si, key)
        if slot != 0
...
function conj_insert!(sc::LP1ConjLSMStore{V}, si::Int,
...
# ---------------------------------------------------------------------------
function conj_insert_or_pop!(sc::LP1ConjLSMStore{V}, si::Int,
    ...
    if bloom_maybe_has(sc.global_bloom, fp)
        for peer in sc.peers
            peer === sc && continue
            peer_lsm = peer::LP1ConjLSMStore{V}
...
function lsm_flush_all!(sc::LP1ConjLSMStore)
    lock(sc.file_lock)
    try
...
function lsm_close!(sc::LP1ConjLSMStore)
    lsm_flush_all!(sc)
    lock(sc.file_lock)
...
function lsm_info(sc::LP1ConjLSMStore; io::IO = stdout)
    ...
    @printf(io, "LP1ConjLSMStore: %d hot (%.1f%% load) | %d disk-live | %d runs | spill=%s (%.1f MB) | cold-dropped=%d\n",
...
function lsm_to_dict(sc::LP1ConjLSMStore{V})::Dict{CanonicalLP1Key, V} where V
...
conj_to_dict(sc::LP1ConjLSMStore{V}) where V = lsm_to_dict(sc)
...
function lsm_snapshot_and_free!(arr::Vector{<:LP1ConjLSMStore{V}};
...
function conj_roundtrip_ok(sc::LP1ConjLSMStore{V}, si::Int,
                            key::CanonicalLP1Key, val::V)::Bool where V
...
function lsm_recommended_hot_cap(sc::LP1ConjLSMStore;
                                  safety_factor::Int = 8
...
function lsm_resize_hot!(sc::LP1ConjLSMStore{V},
    ...
    if verbose && any_resized
        ...
        @printf("[LP1ConjLSMStore] resized hot table → %d entries (%d/shard, thresh=%d)\n",
...
function lsm_autotune!(sc::LP1ConjLSMStore;
                        safety_factor::Int  = 8,
    ...
    if Float64(current) / Float64(needed) < min_shrink_ratio
        verbose && @printf("[LP1ConjLSMStore autotune] no resize: %s\n", reason)
        return false
    end

    verbose && @printf("[LP1ConjLSMStore autotune] resizing: %s\n", reason)
...
# ---------------------------------------------------------------------------
function _lsm_disk_delete!(sc::LP1ConjLSMStore, ri::Int, pos::Int)
    _run_set_dead!(sc.runs[ri], pos)
    sc.n_disk_live -= 1
    nothing
end
...
# ---------------------------------------------------------------------------
function lsm_mem_report(sc::LP1ConjLSMStore{V};
                         label   ::String = "",
                         peers   ::Bool   = false,
    ...
    function _single_lsm_mem(lsm::LP1ConjLSMStore{W}, lbl::String) where W
        @printf(io, "\n[LP1ConjLSMStore memory report%s]\n",
                isempty(lbl) ? "" : "  ($lbl)")
...
# ---------------------------------------------------------------------------
function lsm_flush_stats(sc::LP1ConjLSMStore; io::IO = stdout)
    @printf(io, "\n[LP1ConjLSMStore flush / spill stats]\n")
...
# ---------------------------------------------------------------------------
function lsm_multiplicity_report(peers::Vector{<:LP1ConjLSMStore},
                                  p::Integer;
                                  io::IO = stdout)
