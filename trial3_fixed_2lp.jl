    ...
    if verbose
        ...
        if hits_total > 0
            ...
            @printf("  normalized relation rates: full/valid=%.4f, tree/valid=%.4f, LP-pairs/valid=%.4f\n",
                    hits_full / hits_total, hits_tree / hits_total, hits_lp2emit / hits_total)
        end
...
@inline function LP2Edge(a::NTuple{2,Int}, b::NTuple{2,Int},
                         row::Dict{Int,Int}, alpha::Int, beta::Int)
    a <= b ? LP2Edge(a, b, row, alpha, beta) : LP2Edge(b, a, row, alpha, beta)
end
...
function lp2_attach!(edges::Vector{LP2Edge},
    ...
    # Keep folding the current edge against already-seen edges as long as a
    # live incident edge exists at either endpoint.
    while true
        hit_eid = 0
        hit_pt  = cur_left
        ...
        if old_other === nothing
            live[hit_eid] = false
            continue
        end

        # Consume the old edge and merge its relation with the current edge.
        live[hit_eid] = false
        ...
        # Continue the walk with the new edge connecting the other endpoints.
        e = LP2Edge(old_other, cur_other, cur_row, cur_alpha, cur_beta)
        cur_left, cur_right = e.left, e.right
    end
end
