...
function tree_add_vertex!(tree::LPStageTree, pt::NTuple{2,Int},
    ...
    prow = parent === nothing ? Dict{Int,Int}() : tree.vertices[parent].row
    ...
    row = copy(prow)
    sparse_add!(row, local_row)
    tree.vertices[pt] = TreeVertex(parent, pdep + 1, row,
                                   mod(pa + alpha, ell),
                                   mod(pb + beta, ell))
