...
function main2()
    ...
    # FB size ~ p^(2/3) is the asymptotically optimal choice for genus-2 index calculus.
    fb_size = round(Int, p^(2/3))
    verbose_fb = true
    verbose_fb && @printf("Using fb_size = %d  (≈ p^(2/3), p=%d)\n\n", fb_size, p)

    # Main run: uncapped walk; runs until enough relations are collected.
    k_rec = index_calculus_walk(G, T; fb_size=fb_size,
