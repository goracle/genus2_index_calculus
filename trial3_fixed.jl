...
include("lp2_conj.jl")  # mixed affine/QLP LP2 graph for extension-field LPs
...
function left_kernel_all(rel_rows::Vector{Dict{Int,Int}}, nF::Int, ell::Int)::Vector{Vector{Int}}
    ...
    # We build the matrix cell-by-cell to avoid the n*m dense flat buffer
    # (which at n=10000, m=11000 is ~880MB — the main GC pressure source).
    try
        F = Nemo.GF(ell)
        Rnemo_t = zero_matrix(F, n, m)
        for i in 1:m
            for (j, v) in rel_rows[i]
                if 1 <= j <= n
...
function index_calculus_walk(G::Div2, T::Div2;
    ...
    empty!(all_samples)
    GC.gc()
    ccall((:flint_set_num_threads, :libflint), Cvoid, (Cint,), 1)  # prevent FLINT/Julia pthread deadlock
