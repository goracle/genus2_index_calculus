...
#  representative, and the Jacobian group law handles the [∞] part.)
#
# The walk records two sign conventions depending on the path:
#   0-LP, 1-LP closures: row = combined_fb,  al = combined_al,  be = combined_be
#     and the invariant is  Σ row[j]·atom(fb[j]) == combined_al·G + combined_be·T
#   2-LP even cycles: emitted_rel.alpha/beta from lp2_insert_edge, same invariant
#   lp_doubled cross-close: 2*(neg_al_1·G + neg_be_1·T) - (al_d·G + be_d·T)
#
# Returns true if the FB sum equals al·G + be·T (mod the Jacobian group law),
# false otherwise.  Prints a diagnostic line on failure.
function check_relation_principal(
        row::Dict{Int,Int}, al::Int, be::Int,
        alpha_name::String,   # label for the alpha coefficient (for diagnostics)
    ...
    # Check negated RHS in case the sign convention is flipped.
    D_rhs_neg = jac_neg(D_rhs)
    if D_fb == D_rhs_neg
        # This is the sign-flipped convention — still internally consistent,
        # but print a warning so we know which convention is in use.
...
# Check that a 1-LP stored entry is consistent:
#   Σ row[j]·atom(fb[j]) + atom(lp_pt) == neg_al·G + neg_be·T
# i.e. the row does NOT include lp_pt — it is a pure FB sparse vector
# and lp_pt is the residual unknown on the LHS.
function check_lp1_stored(
        lp_pt::NTuple{2,Int}, row::Dict{Int,Int},
        neg_al::Int, neg_be::Int,
        fb::Vector{NTuple{2,Int}},
        G::Div2, T::Div2;
        tag::String = "")::Bool
...
function phi_residual_mumford(a::Int, b::Int, c::Int,
                               ...
                               u0::Int, u1::Int)::Union{
                                   ...
                                   Tuple{Nothing, Nothing, NTuple{4,Int}},    # (nothing, nothing, (c0,c1,v0,v1)) — RS unsplit
                                   Nothing}
    ...
    if sq === nothing
        # Conjugate pair over Fp^2.
        # Compute v-poly: v_RS(x) = -(a*x^2+b*x+c) mod u_RS(x).
        # Reduce x^2 → -c1_rs*x - c0_rs:
        #   v_RS(x) = -a*(-c1_rs*x - c0_rs) - b*x - c
        #           = (a*c1_rs - b)*x + (a*c0_rs - c)
        v1_rs = fp(fp(a * c1_rs) - b)
        v0_rs = fp(fp(a * c0_rs) - c)
        # Full Mumford key: (u0, u1, v0, v1) uniquely identifies the degree-2 element.
    ...
    while rel_counter[] < rel_target && raw_steps < step_cap
        ...
        if !rs_split
            ...
            if i0 != 0
                ...
                # P0 off-FB: two unknowns (P0 and RS Mumford element).
                # Skipping LP2 insertion — LP2Graph uses NTuple{2,Int} nodes and cannot
                # accommodate the 4-tuple RS key without refactoring.  These steps are
                # simply discarded (counted as 2-LP seen, no emission).
                hits_lp2seen += 1
                cur_pt = fb[rand(1:nF_cur)]
            end
                ...
                if haskey(shared_lp1, lp_pt)
                    ...
                    if !isempty(combined) && !(combined_al == 0 && combined_be == 0)
                        if ASSERT_RELATIONS
                            ...
                            @assert ok "1-LP closure emission failed principal divisor check (see diagnostic above)"
                        end
                        push!(alpha_vec, combined_al)
                        push!(beta_vec,  combined_be)
                        push!(rel_rows,  combined)
            ...
            try
                for (lp_known, lp_other) in ((lp2_a, lp2_b), (lp2_b, lp2_a))
                    ...
                    if haskey(shared_lp1, lp_other)
                        ...
                        if !isempty(combined) && !(combined_al == 0 && combined_be == 0)
                            if ASSERT_RELATIONS
                                ok = check_relation_principal(combined, combined_al, combined_be, "α", fb, G, T; tag="2LP-CROSS-CLOSE")
                                @assert ok "2-LP cross-close emission failed principal divisor check (see diagnostic above)"
                            end
                            push!(alpha_vec, combined_al)
                            push!(beta_vec,  combined_be)
                            push!(rel_rows,  combined)
                        ...
                        if ASSERT_RELATIONS
                            ...
                            if !ok
                                ...
                                @printf("[2LP-CROSS-STORE DIAG]  r_known=%s  na_known=%d  nb_known=%d\n",
                                        string(r_known), na_known, nb_known)
                                @printf("[2LP-CROSS-STORE DIAG]  fb_row_scratch=%s\n", string(fb_row_scratch))
                                @printf("[2LP-CROSS-STORE DIAG]  new_row=%s  new_neg_al=%d  new_neg_be=%d\n",
                                        string(new_row), new_neg_al, new_neg_be)
                                @printf("[2LP-CROSS-STORE DIAG]  neg_al=%d neg_be=%d  lp2_a=%s  lp2_b=%s\n",
                                        neg_al, neg_be, string(lp2_a), string(lp2_b))
                            end
...
function index_calculus_walk(G::Div2, T::Div2;
    ...
    shared_lp2_lock  = ReentrantLock()
    shared_lp_doubled = Dict{NTuple{2,Int}, Tuple{Dict{Int,Int}, Int, Int}}()
    # Conjugate-pair table: keyed by full 4-tuple Mumford (c0,c1,v0,v1), never mixed with shared_lp1.
    shared_lp1_conj  = Dict{NTuple{4,Int}, Tuple{Dict{Int,Int}, Int, Int, Int}}()
