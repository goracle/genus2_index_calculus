...
This is the geometric content already targeted, independently, by
`HyperellipticClassProof.lean`'s in-progress `ordAt_linX_eq_two_of_ramified` —
this statement is *not* a duplicate of that one; it is the correspondingly
global fact ("ramification ⟺ Weierstrass point", as a ⟺, for every point) built
on top of whatever `ordAt_linX_eq_two_of_ramified` and its unramified
counterpart `pointIdeal_linX_not_sq_dvd` (already proved there) supply once
both are in hand. -/
theorem hyperelliptic_ramification_eq_weierstrass
    (hdeg : H.f.natDegree = 5) (P : H.Point) :
