...
/-- **Roadmap §5 step 1.** If `g = c - X t * d` for a variable `t` not
appearing in `c` or `d`, and `d` is `Rdec p ⧸ Ideal.ofList gens`-regular
(a non-zero-divisor on the quotient by the generators processed so far),
then `g` extends `gens` to a longer weakly-regular sequence: `d` itself is
regular on the quotient, and `IsSMulRegular` transfers from `d` to `g` via
the identity `g = c - X t * d`, since `X t * d` and `d` cut out the same
ideal-membership behaviour up to the unit change of variables `t ↦ c/d` in
the quotient (this is where `t ∉ vars c, d` is used: `X t` is a fresh
polynomial generator, so multiplying by it doesn't introduce any relation
`d` doesn't already have on the quotient). Stated directly in terms of
`IsSMulRegular` (`Mathlib.RingTheory.Regular.IsSMulRegular`) against
`Rdec p ⧸ Ideal.ofList gens` as an `Rdec p ⧸ Ideal.ofList gens`-module over
itself, matching the shape `RingTheory.Sequence.isRegular_cons_iff` actually
consumes (`IsRegular M (r :: rs) ↔ IsSMulRegular M r ∧ IsRegular
(QuotSMulTop r M) rs`) -- chosen over the earlier looser pointwise
formulation so this composes directly with `decoupledSystem_isRegularSequence`'s
goal instead of needing a second translation lemma. Deliberately stated over
an arbitrary `CommRing`, no field/characteristic hypothesis, matching the
roadmap's claim that this step is entirely curve- and `p`-independent.

Genuinely open here (the `sorry`): the actual derivation of
`IsSMulRegular (Rdec p ⧸ I) g` from `IsSMulRegular (Rdec p ⧸ I) d` +
`htc`/`htd`. Candidate route: since `t ∉ c.vars ∪ d.vars`, the ring
`Rdec p` is (as an `Rdec p`-algebra, forgetting `X t`) a polynomial ring in
`X t` over the subring generated by everything but `t`, i.e.
`Rdec p ≃ (MvPolynomial {u // u ≠ t} R)[X t]`-shaped (`MvPolynomial`'s
own variable-restriction/`rename` API, e.g. `MvPolynomial.finSuccEquiv`-style
lemmas but for an arbitrary removed variable rather than `Fin`) -- under that
identification `g` is literally `d`'s image times `-X t` plus the constant
`c`, and `IsSMulRegular` for a "leading coefficient times fresh variable
plus constant" shape should reduce to `d`'s own regularity via the
polynomial ring's `Polynomial.smul_eq_zero`-style zero-divisor structure.
Left as a `sorry` pending exactly which `MvPolynomial` variable-restriction
lemma matches -- flagged as this file's best candidate for a Mathlib-API
web search before the next REPL attempt, not yet done this pass. -/
theorem regular_of_linear_elim {σ : Type*} [DecidableEq σ] {R : Type*} [CommRing R]
    (t : σ) (gens : List (MvPolynomial σ R)) (c d : MvPolynomial σ R)
    (htc : t ∉ c.vars) (htd : t ∉ d.vars)
    (hd_reg : IsSMulRegular (MvPolynomial σ R ⧸ Ideal.ofList gens) d)
    (g : MvPolynomial σ R) (hg : g = c - MvPolynomial.X t * d) :
    IsSMulRegular (MvPolynomial σ R ⧸ Ideal.ofList gens) g := by
  sorry
