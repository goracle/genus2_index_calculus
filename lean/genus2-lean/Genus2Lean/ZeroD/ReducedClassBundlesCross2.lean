...
/-! # `Cross2AssemblyData` bundle for `reducedClass_eq_of_isReduction'_cross2`
...
`AlphaLocusDegreeUniformCross2.lean`'s own four itemized differences
...
1. `sa.P2` is no longer a free point in the CA-witness-facing block — it
   is forced to `Point.iota Ra` via `hP2eq`, so `Cross2AssemblyData` has
   no `as_Ra1`/second free anchor-vs-target point the way `SplitAssemblyData`
   does; the anchor pair is named `as_Ra`/`as_Ra2` (matching that file's
   own `Ra`/`Ra2` naming, not `Ra1`/`Ra2`, to keep the "the point that
   collides is `Ra`, unprimed" convention visible in the bundle too).
2. `caCross2InterpMatrix`/`caCross2Coeff`/`bCACross2`/`uCANewCross2`/
   `denomPolyCACross2` (three free points `RaX,Ra2X,P1X` plus derivative
...
3. `h1P1,h1P2,h2P1,h2P2,hPP` collapse to the three-hypothesis family
   `h1,h2,h3` (`RaX≠Ra2X`, `RaX≠P1X`, `Ra2X≠P1X`).
4. `hP2_curve`/`hP2Y_ne` are gone (no free `sa.P2` to state them about);
   `hP2Deriv` (the derivative datum's defining equation) replaces them.
...
-- types (uCANewCross2/bCACross2/caCross2InterpMatrix applications repeated
...
/-- **Construction-specific assembly layer for the cross2
(`sa.P2 = ι(Ra)`) `reducedClass_eq_of_isReduction'_cross2` theorem.**
...
structure Cross2AssemblyData
  ...
  hT1Y_ne : as_T1.Y ≠ 0
  hT2Y_ne : as_T2.Y ≠ 0
  -- **The cross2 `CAWitness` construction (item 2 of the diff)**: three
  -- free points `as_Ra.X,as_Ra2.X,sa.P1.X` plus `vDerivAtP2`, replacing
  -- `caInterpMatrix`/`uCANew`/`bCA`/`denomPolyCA`'s four-free-point forms.
  as_vDerivAtP2 : F p
  hdet : (caCross2InterpMatrix as_Ra.X as_Ra2.X sa.P1.X).det ≠ 0
  hlead : caCross2Coeff as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2 3 ≠ 0
  h1 : as_Ra.X ≠ as_Ra2.X
  h2 : as_Ra.X ≠ sa.P1.X
  h3 : as_Ra2.X ≠ sa.P1.X
  ...
  hP2Deriv : 2 * as_Ra.Y * (-as_vDerivAtP2) = (derivative H.f).eval as_Ra.X
  hP1Y_ne : sa.P1.Y ≠ 0
  hne : H.f - (bCACross2 as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2) ^ 2 ≠ 0
  hU_evalRa :
      (uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y
        as_vDerivAtP2).eval as_Ra.X ≠ 0
  hU_evalRa2 :
      (uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y
        as_vDerivAtP2).eval as_Ra2.X ≠ 0
  hU_evalP1 :
      (uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y
        as_vDerivAtP2).eval sa.P1.X ≠ 0
  hU_ne0 : uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2 ≠ 0
  as_PtT1 : H.Point
  as_PtT2 : H.Point
  hPtT1X : as_PtT1.X ≠ as_PtT2.X
  hPtT1 :
      (uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y
        as_vDerivAtP2).IsRoot as_PtT1.X
  hPtT2 :
      (uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y
        as_vDerivAtP2).IsRoot as_PtT2.X
  as_Q1 : Polynomial (F p)
  as_Q2 : Polynomial (F p)
  hQ1_def : uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2 =
      (Polynomial.X - Polynomial.C as_PtT1.X) * (Polynomial.X - Polynomial.C as_PtT2.X) * as_Q1
  hQ1T1 : as_Q1.eval as_PtT1.X ≠ 0
  hQ2_def : uCANewCross2 H as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2 =
      (Polynomial.X - Polynomial.C as_PtT2.X) * (Polynomial.X - Polynomial.C as_PtT1.X) * as_Q2
  hQ2T2 : as_Q2.eval as_PtT2.X ≠ 0
  hAeval1 : (denomPolyCACross2 as_Ra.X as_Ra2.X sa.P1.X : Polynomial (F p)).eval as_PtT1.X ≠ 0
  hAeval2 : (denomPolyCACross2 as_Ra.X as_Ra2.X sa.P1.X : Polynomial (F p)).eval as_PtT2.X ≠ 0
  hPtT1Y :
      as_PtT1.Y = (bCACross2 as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y
        as_vDerivAtP2).eval as_PtT1.X
  hPtT1Y_ne : as_PtT1.Y ≠ 0
  hPtT2Y :
      as_PtT2.Y = (bCACross2 as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y
        as_vDerivAtP2).eval as_PtT2.X
  hPtT2Y_ne : as_PtT2.Y ≠ 0
  hRaT1 : as_Ra.X ≠ as_PtT1.X
  hRaT2 : as_Ra.X ≠ as_PtT2.X
  hRa2T1 : as_Ra2.X ≠ as_PtT1.X
  hRa2T2 : as_Ra2.X ≠ as_PtT2.X
  ...
  hsupp_f : ∀ P, P ∉ ({as_Ra, as_Ra2, Point.iota sa.P1, as_PtT1, as_PtT2} :
      Finset H.Point) →
    ordAt P (-bCACross2 as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2)
      (1 : Polynomial (F p)) = 0
  hspec_f : ∀ vv : IsDedekindDomain.HeightOneSpectrum (CoordinateRing H),
    (Associates.mk vv.asIdeal).count (Associates.mk (Ideal.span
      ({toPair H (-bCACross2 as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2) 1} :
        Set (CoordinateRing H)))).factors ≠ 0 → ∃ P, vv.asIdeal = pointIdeal P
  [hspec_f_finite : ∀ P : ({as_Ra, as_Ra2, Point.iota sa.P1, as_PtT1, as_PtT2} : Finset H.Point),
    Module.Finite (F p) (CoordinateRing H ⧸ pointIdeal P.1 ^
      (ordAt P.1 (-bCACross2 as_Ra.X as_Ra2.X sa.P1.X as_Ra.Y as_Ra2.Y sa.P1.Y as_vDerivAtP2)
...
-- Bumped for the same reason as Cross2AssemblyData above: the proof
-- body threads ~90 hypothesis/field references through several
-- multi-argument callee applications (cAmιTmδmιδ_mem_of_le_cross2 alone
...
set_option maxHeartbeats 1000000 in
/-- **Cross2 theorem, bundled.** Mirrors `ReducedClassBundles.lean`'s
bundled base theorem, taking `(base : ReductionData sa) (d :
Cross2AssemblyData sa)` instead of the ~90-hypothesis flat signature.
Proof body is `AlphaLocusDegreeUniformCross2.lean`'s original proof with
every free variable replaced by its `d.`-projection, `hP2eq` handled
identically via `d.hP2eq` at the same point in the argument. -/
theorem reducedClass_eq_of_isReduction'_cross2 {p : ℕ} [Fact (Nat.Prime p)] [Fact (p ≠ 2)]
  ...
  classical
  have hRa12ne : d.as_Ra ≠ d.as_Ra2 := fun h => d.hRa12Xne (by rw [h])
  have hSanchorEq : d.as_Sanchor = ({d.as_Ra, d.as_Ra2} : Finset H.Point) :=
    Sanchor_eq_of_anchor_roots d.as_ua d.as_va d.hua d.huafree
      d.as_Ra d.as_Ra2 hRa12ne d.hRa12Xne ⟨d.hRaRoot, d.hRa2Root⟩ d.hRaY d.hRa2Y d.as_Sanchor
  ...
  have hSanchorSum : divToPair (H := H) (-d.as_va) 1 d.as_Sanchor
      = single d.as_Ra + single d.as_Ra2 :=
    DecoupledSystem.divToPair_negVa_one_Sanchor_eq (H := H) d.hchar
      ...
      d.hUco_ne
      d.as_Ra d.as_Ra2 hRa12ne d.hRaY_ne d.hRa2Y_ne hRaRoot' hRa2Root' d.hRaY d.hRa2Y
      d.hUco_evalRa d.hUco_evalRa2 d.as_Sanchor hSanchorEq
  ...
  have hDP := cAmιTmδmιδ_mem_of_le_cross2 (H := H) hdeg d.hchar d.hsf D hD
    d.hspec_linX
    d.as_Ra.X d.as_Ra2.X sa.P1.X d.as_Ra.Y d.as_Ra2.Y sa.P1.Y d.as_vDerivAtP2
    d.hdet d.hlead d.h1 d.h2 d.h3
    d.hRa_curve d.hRa2_curve d.hP1_curve d.hP2Deriv d.hRaY_ne d.hRa2Y_ne d.hP1Y_ne
    d.as_Ra d.as_Ra2 sa.P1 (Point.iota sa.P1)
    rfl rfl rfl rfl rfl rfl rfl rfl
    d.hne
    d.hU_evalRa d.hU_evalRa2 d.hU_evalP1 d.hU_ne0
    ...
    d.hRaT1 d.hRaT2 d.hRa2T1 d.hRa2T2 d.hP1T1 d.hP1T2
  ...
  have hcoe : ((aAnchor - aP2P1Nι : Divisor0 H) : Divisor H) -
      ...
      (single d.as_Ra + single d.as_Ra2 - single sa.P2 - single sa.P1 -
