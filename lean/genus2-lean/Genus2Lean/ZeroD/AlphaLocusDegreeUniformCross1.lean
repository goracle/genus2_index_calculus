...
/-- **Cross variant 1 top-level wiring**: `sa.reducedClass + q = toJacobian D
(target)`, for the case `sa.P1 = ι(Ra)` (`Ra`'s own conjugate collides with
the target's first point). Signature is `reducedClass_eq_of_isReduction'`'s
own, with `Ra1` renamed `Ra`, `sa.P1` forced via `hP1eq`, the six-hypothesis
