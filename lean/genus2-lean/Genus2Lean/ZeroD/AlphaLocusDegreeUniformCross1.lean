...
/-! # Cross-pair tangent (`Ra1 = ι(sa.P1)`) sibling of
`reducedClass_eq_of_isReduction'`

`ROADMAP-cawitness-tangent-interpolation.md`, Part B, case 3, step 5's
"still open" item: the top-level theorem consuming `cAmιTmδmιδ_mem_of_le_cross1`
...
(`caCrossInterpMatrix`'s own `h1 : RaX ≠ Ra2X` — checked directly,
`CAWitnessCrossTangent2.lean`) — the confluence is between the anchor
point `Ra` and the TARGET point `sa.P1`, via `sa.P1 = Point.iota Ra` (the
...
2. `caInterpMatrix`/`caCoeff`/`bCA`/`uCANew`/`denomPolyCA` (four free
   points `Ra1,Ra2,P1,P2`) are replaced by `caCrossInterpMatrix`/
   `caCrossCoeff`/`bCACross`/`uCANewCross`/`denomPolyCACross` (three free
   points `RaX,Ra2X,P2X` plus the derivative datum `vDerivAtP1` —
   `CAWitnessCrossTangent2.lean`), matching `cAmιTmδmιδ_mem_of_le_cross1`'s
   own signature exactly.
3. `h1P1,h1P2,h2P1,h2P2,hPP` (six-hypothesis-family's cross/target members)
   disappear entirely — cross1's construction only needs `h1,h2,h3`
   (`RaX≠Ra2X`, `RaX≠P2X`, `Ra2X≠P2X`), the three-hypothesis nondegeneracy
...
/-- **Cross variant 1 top-level wiring, FLAT-signature original.**
`sa.reducedClass + q = toJacobian D (target)`, for the case `sa.P1 = ι(Ra)`
(`Ra`'s own conjugate collides with the target's first point). Signature is
`reducedClass_eq_of_isReduction'`'s own, with `Ra1` renamed `Ra`, `sa.P1`
forced via `hP1eq`, the six-hypothesis family collapsed to `h1,h2,h3`, and
the CA-witness functions swapped for their `Cross`-suffixed siblings; see
the module docstring for the itemized diff.

Renamed from `reducedClass_eq_of_isReduction'_cross1` to `..._cross1_flat`
(per `ROADMAP-reducedClass-dispatcher.md`'s bundling work): the bundled
version in `ReducedClassBundlesCross1.lean` now owns the unqualified name,
matching the convention `_tangent`/`_tangent_target` already established
(bundled-only, no separate flat sibling kept under the plain name). This
flat theorem is grep-confirmed to have no external callers, so the rename
is safe; kept (rather than deleted) as a second, independently-checkable
proof route to the same conclusion. -/
theorem reducedClass_eq_of_isReduction'_cross1_flat {p : ℕ} [Fact (Nat.Prime p)] [Fact (p ≠ 2)]
    ...
    -- `caCrossInterpMatrix` construction's own convention
    -- (`CAWitnessCrossTangent2.lean`, `Ra1 = ι(sa.P1)`).
    (Ra Ra2 : H.Point)
    (hRa12Xne : Ra.X ≠ Ra2.X)
    ...
    -- **The cross-pair `CAWitness` identification**: `caCrossInterpMatrix`'s
    -- own three free points `RaX,Ra2X,P2X` (`sa.P1` is gone, replaced by
    -- the `ι(Ra)`-identification `hP1eq` above), plus the target-derivative
    -- datum `vDerivAtP1` (`= -(derivative v).eval sa.P1.X`, per
    -- `CAWitnessCrossTangent2.lean`'s own `hP1Deriv` convention).
    (vDerivAtP1 : F p)
    (hdet : (caCrossInterpMatrix Ra.X Ra2.X sa.P2.X).det ≠ 0)
    (hlead : caCrossCoeff Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y 3 ≠ 0)
    (h1 : Ra.X ≠ Ra2.X) (h2 : Ra.X ≠ sa.P2.X) (h3 : Ra2.X ≠ sa.P2.X)
    ...
    (hP2Y_ne : sa.P2.Y ≠ 0)
    (hne : H.f - (bCACross Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y) ^ 2 ≠ 0)
    (hU_evalRa : (uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y).eval Ra.X ≠ 0)
    (hU_evalRa2 : (uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y).eval Ra2.X ≠ 0)
    (hU_evalP2 : (uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y).eval sa.P2.X ≠ 0)
    (hU_ne0 : uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y ≠ 0)
    (PtT1 PtT2 : H.Point)
    (hPtT1X : PtT1.X ≠ PtT2.X)
    (hPtT1 : (uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y).IsRoot PtT1.X)
    (hPtT2 : (uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y).IsRoot PtT2.X)
    (Q1 Q2 : Polynomial (F p))
    (hQ1_def : uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y =
      (Polynomial.X - Polynomial.C PtT1.X) * (Polynomial.X - Polynomial.C PtT2.X) * Q1)
    (hQ1T1 : Q1.eval PtT1.X ≠ 0)
    (hQ2_def : uCANewCross H Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y =
      (Polynomial.X - Polynomial.C PtT2.X) * (Polynomial.X - Polynomial.C PtT1.X) * Q2)
    (hQ2T2 : Q2.eval PtT2.X ≠ 0)
    (hAeval1 : (denomPolyCACross Ra.X Ra2.X sa.P2.X : Polynomial (F p)).eval PtT1.X ≠ 0)
    (hAeval2 : (denomPolyCACross Ra.X Ra2.X sa.P2.X : Polynomial (F p)).eval PtT2.X ≠ 0)
    (hPtT1Y : PtT1.Y = (bCACross Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y).eval PtT1.X)
    (hPtT1Y_ne : PtT1.Y ≠ 0)
    (hPtT2Y : PtT2.Y = (bCACross Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y).eval PtT2.X)
    (hPtT2Y_ne : PtT2.Y ≠ 0)
    ...
    (hsupp_f : ∀ P, P ∉ ({Ra, Ra2, Point.iota sa.P2, PtT1, PtT2} :
        Finset H.Point) →
      ordAt P (-bCACross Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y) (1 : Polynomial (F p)) = 0)
    (hspec_f : ∀ vv : IsDedekindDomain.HeightOneSpectrum (CoordinateRing H),
      (Associates.mk vv.asIdeal).count (Associates.mk (Ideal.span
        ({toPair H (-bCACross Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y) 1} :
          Set (CoordinateRing H)))).factors ≠ 0 → ∃ P, vv.asIdeal = pointIdeal P)
    [∀ P : ({Ra, Ra2, Point.iota sa.P2, PtT1, PtT2} : Finset H.Point),
      Module.Finite (F p) (CoordinateRing H ⧸ pointIdeal P.1 ^
        (ordAt P.1 (-bCACross Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y)
  ...
  -- **The cross1 concrete-coordinate assembly (†)**: `[Ra]+[Ra2]-[ιRa] -
  -- [ιT1]-[ιT2] - [δ₀]-[ιδ₀]`-shaped fact, then rewritten below (via
  -- `hcoe`) into `[Ra]+[Ra2]-[ι(Ra)]-[sa.P2]-...` matching this theorem's
  -- own `single Ra1 + single Ra2 - single sa.P1 - single sa.P2 - ...`
  -- shape once `hP1eq` identifies `single sa.P1 = single (Point.iota Ra)`.
  have hDP := cAmιTmδmιδ_mem_of_le_cross1 (H := H) hdeg hchar hsf D hD
    hspec_linX
    Ra.X Ra2.X sa.P2.X Ra.Y Ra2.Y vDerivAtP1 sa.P2.Y
    hdet hlead h1 h2 h3
    hRa_curve hRa2_curve hP2_curve hP1Deriv hRaY_ne hRa2Y_ne hP2Y_ne
    Ra Ra2 sa.P2 (Point.iota sa.P2)
    rfl rfl rfl rfl rfl rfl rfl rfl
    hne
    hU_evalRa hU_evalRa2 hU_evalP2 hU_ne0
  ...
  have hcoe : ((aAnchor - aP1P2Nι : Divisor0 H) : Divisor H) -
      ...
      (single Ra + single Ra2 - single sa.P1 - single sa.P2 -
