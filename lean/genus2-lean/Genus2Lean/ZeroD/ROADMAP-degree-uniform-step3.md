...
**Update, this pass — the `sa ≠ sb` test is now settled, via ChatGPT
consult (not yet formalized in Lean): case (b), the BAD case, but with
a clean, cheap, exploitable structure.** The four cross-resultants
`P_{u0},P_{u1},P_{v0},P_{v1}` are **not** in the ideal generated by the
4 curve relations — their vanishing is a genuine matching condition
(equality of the two samples' divisor classes), not an algebraic
identity. **However, the four equations are not independent**: on the
generic Mumford chart they impose only **codimension 2** (not 4), so
the matching locus inside the 4-dimensional curve-relation variety is
expected to be exactly **2-dimensional**. This lines up well with this
project's own earlier-noted "2-dimensional space of 0-dimensional
fibers" picture (see this file's own "Newer update" section at the
top, and `ROADMAP-alpha-locus.md`) — an independent confirmation from a
different angle, not a contradiction.

**The consult's most Lean-actionable finding**: don't expand the
Cramer determinants at all. `u1,u0` have a trivial closed form
independent of the Cramer machinery — `u1 = -(a1+a2)`, `u0 = a1·a2`
(elementary symmetric functions of the two roots, straight from
`u(x)=(x-a1)(x-a2)`) — and by uniqueness of the linear system's
solution, the Cramer ratio **equals** this closed form on the
determinant-nonzero locus, so a Lean proof can go via "prove the
explicit formula solves the same defining linear system, invoke
uniqueness" rather than ever unfolding `Matrix.cramer`. Likewise
`v1 = (wa1-wa2)/(a1-a2)`, `v0 = (a1·wa2 - a2·wa1)/(a1-a2)` (standard
linear interpolation through the two anchor points), same trick.
`P_{u1} = D_{u1}^{(a)}·D_{u1}^{(b)}·(u1^{(a)}-u1^{(b)})` reduces
immediately, on the nonzero-denominator locus, to `D^{(a)}·D^{(b)}·
((b1+b2)-(a1+a2))` — visibly nonzero generically, which is itself the
one-line proof that `P_{u1} ∉ I_curve` (no Gröbner basis needed for
the non-identity half).

**Geometric content, for the eventual Lean statement**: once `u0,u1`
agree, `(x-a1)(x-a2)=(x-b1)(x-b2)` forces `{b1,b2}={a1,a2}` generically
over the closure, and then `v0,v1` agreeing forces the `w`-coordinates
to match too — so the codimension-2 locus is exactly "the two samples
are the same unordered point-pair on the curve" (up to swap), matching
the "same Jacobian class" reading. **One exceptional-case caveat the
consult flagged, worth remembering before stating a final theorem**:
this is about *raw* degree-2 divisors, not their unique reduced Mumford
form — a conjugate pair `P + ι(P)` is linearly equivalent to `2·∞` but
has different `u(x)=(x-a)²` as `a` varies, so on that exceptional locus
"cross-resultants vanish" is strictly stronger than "same divisor
class." The clean statement is scoped to the open locus of non-special
degree-2 reduced representatives with nonzero Cramer denominators.

**Recommended before trusting any specific numeral from a
Gröbner/dimension computation**: saturate `(I_curve + (P_{u0},...,
P_{v1})) : D^∞` where `D` is the product of all 8 Cramer denominators
— the raw (unsaturated) variety can carry spurious extra components
purely from denominator-vanishing loci, which would make a naive
dimension count come out wrong.

**Net effect on Obligation 1**: (c) is now understood, not open — but
it settles to the WORSE of the two possible outcomes flagged above:
"characterize + work around a real constraint on `(sa,sb)`," not
"prove genericity is automatic." The constraint has a clean
description (same divisor class / same point-pair) though, so the
actual remaining Lean task is bounded: exhibit ONE witness `(sa,sb)`
pair where (a) curve relations hold, (b) denominators are nonzero, AND
(c) `sa,sb` represent the same divisor class (trivially satisfiable —
e.g. literally take `sa = sb`, already covered by the earlier symmetric
result, OR any two point-orderings of one honest curve divisor) —
this is much narrower than "prove genericity over the whole 4D
variety," since (c) is telling us WHICH `(sa,sb)` to pick, not that
almost all of them fail. **Not yet formalized in Lean this pass** —
the consult is a mathematical resolution, the concrete next step is
translating "pick `sa,sb` representing the same class" into an actual
Lean witness construction and checking it against `theData`'s literal
denominator-nonvanishing needs for (b).
