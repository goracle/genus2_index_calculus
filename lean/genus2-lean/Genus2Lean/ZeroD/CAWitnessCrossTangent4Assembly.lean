...
/-!
# Cross-pair variant 4 (`Ra2 = ι(sa.P2)`) assembly: the divisor identity
# and `principalSubgroup` membership

Per `ROADMAP-cawitness-tangent-interpolation.md`'s Part B, case 3 /
Suggested-order item 5: `CAWitnessCrossTangentV4.lean` supplies the raw
`bCACross4`/`uCANewCross4` polynomial machinery for the `Ra2X = P2X`
collision (`Ra2 = ι(sa.P2)` as points), but — per that doc's own
...
closes that gap for THIS variant (the direct mirror of
`CAWitnessCrossTangent3Assembly.lean`'s `Ra2 = ι(sa.P1)` case), following
the same `CAWitnessDivisorTangent.lean` + `CAWitnessAssemblyTangent.lean`
+ `PrincipalWitnessStep4Tangent.lean`-style pattern collapsed into one
file.

**Naming convention, matching `CAWitnessCrossTangentV4.lean`**:
`Ra1X` (ordinary anchor row 0), `RaX` (doubled node, `= Ra2X = sa.P2.X`),
`P1X` (ordinary target row 2, survives as `ιP1` — note V4's surviving
target is `P1`, unlike V3 whose surviving target was `P2`; the doubled
node's derivative data is `vDerivAtP2`, matching V4's own row-3
confluent slot). Support set: three points pre-residual
(`Ra1, Ra (mult 2), ιP1`), degree `1+2+1 = 4`, matching
`denomPolyCACross4`'s degree-4 denominator; the residual then adds
`T1, T2` for a five-point total support, same shape as V3 and the
anchor/target tangent cases.

**RHS sign at `Ra`'s row**: per `CAWitnessCrossTangentV4.lean`'s module
...
/-- **`bCACross4`'s pole order at infinity: `-6`.** Same weakened shape
as `bCA_ordInfOfPair`/`bCATangent_ordInfOfPair`: needs
`caCross4Coeff ... 3 ≠ 0`. -/
theorem bCACross4_ordInfOfPair (Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 : k)
    (hlead : caCross4Coeff Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 3 ≠ 0) :
    ordInfOfPair (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = -6 := by
  have hdeg3 : (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).natDegree = 3 := by
    apply le_antisymm (bCACross4_natDegree_le Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2)
    unfold bCACross4
    have hcoeff3 : (Polynomial.C (caCross4Coeff Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 0) +
      ...
      Polynomial.C (caCross4Coeff Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 3) *
        X ^ 3).coeff 3 =
        caCross4Coeff Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 3 := by
      simp [coeff_add, coeff_C_mul, coeff_X_pow]
    exact Polynomial.le_natDegree_of_ne_zero (by rw [hcoeff3]; exact hlead)
  have hAdeg : (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).natDegree = 3 := by
    rw [natDegree_neg]; exact hdeg3
  unfold ordInfOfPair
  rw [hAdeg, natDegree_one]
  norm_num

/-- **Local Layer-3-cross4, `Ra1` side: `ordAt Q denomPolyCACross4 0 =
1` at `Ra1`.** `denomPolyCACross4 = (X-C Ra1X) * (X-C RaX)^2 * (X-C
P1X)`; at `Q` with `Q.X = Ra1X`: the linear factor contributes `1`, the
squared factor and the other linear factor each contribute `0`. -/
theorem ordAt_denomCACross4_eq_one_at_Ra1
    (hchar : (2 : k) ≠ 0) (hsf : Squarefree H.f)
    (Ra1X RaX P1X : k) (h1 : Ra1X ≠ RaX) (h2 : Ra1X ≠ P1X)
    (Q : H.Point) (h_bot : pointIdeal Q ≠ ⊥) (hQX : Q.X = Ra1X) (hQY : Q.Y ≠ 0) :
    ordAt Q (denomPolyCACross4 Ra1X RaX P1X) (0 : k[X]) = 1 := by
  have heq : denomPolyCACross4 Ra1X RaX P1X =
      (linX Ra1X * (linX RaX) ^ 2) * linX P1X := by
    unfold denomPolyCACross4 linX; ring
  rw [heq]
  ...
  have hstep1 : ordAt Q (linX Ra1X * (linX RaX) ^ 2) (0 : k[X]) = 1 := by
    ...
    norm_num
  rw [ordAt_add_of_pairNorm_eq_mul Q h_bot ((linX Ra1X * (linX RaX) ^ 2) * linX P1X)
    (linX Ra1X * (linX RaX) ^ 2) (linX P1X) rfl h1_ne hF2_ne, hstep1, hF2]
  norm_num

/-- **Local Layer-3-cross4, doubled-node side: `ordAt Q denomPolyCACross4
0 = 2` at `Ra` (`= Ra2X = P1X`).** -/
theorem ordAt_denomCACross4_eq_two_at_Ra
    (hchar : (2 : k) ≠ 0) (hsf : Squarefree H.f)
    (Ra1X RaX P1X : k) (h1 : Ra1X ≠ RaX) (h3 : RaX ≠ P1X)
    (Q : H.Point) (h_bot : pointIdeal Q ≠ ⊥) (hQX : Q.X = RaX) (hQY : Q.Y ≠ 0) :
    ordAt Q (denomPolyCACross4 Ra1X RaX P1X) (0 : k[X]) = 2 := by
  have heq : denomPolyCACross4 Ra1X RaX P1X =
      (linX Ra1X * (linX RaX) ^ 2) * linX P1X := by
    unfold denomPolyCACross4 linX; ring
  rw [heq]
...
/-- **Local Layer-3-cross4, `P1` side: `ordAt Q denomPolyCACross4 0 = 1`
at `P1`.** -/
theorem ordAt_denomCACross4_eq_one_at_P1
    (hchar : (2 : k) ≠ 0) (hsf : Squarefree H.f)
    (Ra1X RaX P1X : k) (h2 : Ra1X ≠ P1X) (h3 : RaX ≠ P1X)
...
/-- **`f`'s divisor restricted to the three named points, cross4 case:
`[Ra1] + 2•[Ra] + [ιP1]`.** `f := toPair H (-bCACross4) 1`. `Ra1` is the
ordinary surviving anchor point; `Ra` is the doubled node
(`Ra2 = ι(sa.P2)`, `Ra.Y = RaY` unflipped per `CAWitnessCrossTangentV4.
lean`'s module docstring — `Ra` is read here as an anchor point, not as
`ι(sa.P2)`, matching `bCACross4_eval_Ra`'s own unflipped RHS); `PtιP1`
is `sa.P1`'s hyperelliptic conjugate, flipped as usual. Mirrors
`divToPair_eq_C_add_iotaA_of_split_tangent` exactly, with the doubled
node's `hOrdRa` proof step (`ordAt = 2`) using `bCACross4_eval_Ra`'s
...
theorem divToPair_eq_C_add_iotaA_of_split_cross4
    [DecidableEq H.Point]
    (hchar : (2 : k) ≠ 0) (hsf : Squarefree H.f)
    ...
    divToPair (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X])
        ({PtRa1, PtRa, PtιP1} : Finset H.Point) =
      single PtRa1 + (2 : ℤ) • single PtRa + single PtιP1 := by
  have h_bot : ∀ P : H.Point, pointIdeal P ≠ ⊥ := pointIdeal_ne_bot
  have hg_ne : toPair H (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) ≠ 0 := by
    rw [Ne, toPair_eq_zero_iff]
    exact fun ⟨_, hB⟩ => one_ne_zero hB
  have hAU : pairNorm H (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) =
      denomPolyCACross4 Ra1X RaX P1X *
        (-uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) := by
    unfold pairNorm
    have hfact := pairNormBCACross4_eq_denomPolyCACross4_mul_uCANewCross4 H
      Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hdet h1 h2 h3
      hRa1_curve hRa_curve hP1_curve hP1Deriv hne
    have : (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) ^ 2 - (1 : k[X]) ^ 2 * H.f =
        -(H.f - (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) ^ 2) := by ring
    rw [neg_sq, this, hfact]
    ring
  have hdenom_ne : denomPolyCACross4 Ra1X RaX P1X ≠ 0 := by
    unfold denomPolyCACross4
    exact mul_ne_zero (mul_ne_zero (X_sub_C_ne_zero Ra1X) (pow_ne_zero 2 (X_sub_C_ne_zero RaX)))
      (X_sub_C_ne_zero P1X)
  have hA_ne : toPair H (denomPolyCACross4 Ra1X RaX P1X) (0 : k[X]) ≠ 0 := by
    rw [Ne, toPair_eq_zero_iff]
    exact fun ⟨hA, _⟩ => hdenom_ne hA
  have hU_ne : toPair H (-uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (0 : k[X]) ≠ 0 := by
    rw [Ne, toPair_eq_zero_iff]
    rintro ⟨hA, -⟩
    have : (uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).eval Ra1X = 0 := by
      rw [neg_eq_zero] at hA; rw [hA]; simp
    exact hU_evalRa1 this
  -- Ordinary anchor point `Ra1`, `ordAt = 1`.
  have hOrdRa1 : ordAt PtRa1 (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 1 := by
    apply ordAt_eq_one_of_old_point PtRa1 (h_bot PtRa1) _ _
      (denomPolyCACross4 Ra1X RaX P1X) (-uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2)
      hg_ne _ hAU hA_ne hU_ne _ _
    · show (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).eval PtRa1.X +
          (-(1 : k[X])).eval PtRa1.X * PtRa1.Y ≠ 0
      simp only [hPtRa1X, hPtRa1Y, eval_neg, eval_one]
      rw [bCACross4_eval_Ra1 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hdet]
      intro hcontra
      apply hRa1Y_ne
      ...
      · exact h
    · exact ordAt_denomCACross4_eq_one_at_Ra1 hchar hsf Ra1X RaX P1X h1 h2
  ...
  have hOrdRa : ordAt PtRa (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 2 := by
    apply ordAt_eq_two_of_old_point PtRa (h_bot PtRa) _ _
      (denomPolyCACross4 Ra1X RaX P1X) (-uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2)
      hg_ne _ hAU hA_ne hU_ne _ _
    · show (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).eval PtRa.X +
          (-(1 : k[X])).eval PtRa.X * PtRa.Y ≠ 0
      simp only [hPtRaX, hPtRaY, eval_neg, eval_one]
      rw [bCACross4_eval_Ra Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hdet]
      intro hcontra
      apply hRaY_ne
      ...
      · exact absurd h hchar
      · exact h
    · exact ordAt_denomCACross4_eq_two_at_Ra hchar hsf Ra1X RaX P1X h1 h3
  ...
  have hOrdιP1 : ordAt PtιP1 (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 1 := by
    apply ordAt_eq_one_of_old_point PtιP1 (h_bot PtιP1) _ _
      (denomPolyCACross4 Ra1X RaX P1X) (-uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2)
      hg_ne _ hAU hA_ne hU_ne _ _
    · show (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).eval PtιP1.X +
          (-(1 : k[X])).eval PtιP1.X * PtιP1.Y ≠ 0
      simp only [hPtιP1X, hPtιP1Y, eval_neg, eval_one]
      rw [bCACross4_eval_P1 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hdet]
      intro hcontra
      apply hP1Y_ne
      ...
      · exact absurd h hchar
      · exact h
    · exact ordAt_denomCACross4_eq_one_at_P1 hchar hsf Ra1X RaX P1X h2 h3
  ...
  have hRa1ιP1' : PtRa1 ≠ PtιP1 := hne_of_X (hPtRa1X ▸ hPtιP1X ▸ h2)
  have hRaιP1' : PtRa ≠ PtιP1 := hne_of_X (hPtRaX ▸ hPtιP1X ▸ h3)
  ...
  · rw [hEqRa1]
    simpa [hOrdRa1, hRa1Ra', hRa1ιP1']
  by_cases hEqRa : P = PtRa
  · rw [hEqRa]
    have hRaRa1' : PtRa ≠ PtRa1 := hRa1Ra'.symm
    simpa [hOrdRa, hRaRa1', hRaιP1']
  by_cases hEqιP1 : P = PtιP1
  · rw [hEqιP1]
    have hιP1Ra1' : PtιP1 ≠ PtRa1 := hRa1ιP1'.symm
    have hιP1Ra' : PtιP1 ≠ PtRa := hRaιP1'.symm
    simpa [hOrdιP1, hιP1Ra1', hιP1Ra']
  · have hnmemS : P ∉ ({PtRa1, PtRa, PtιP1} : Finset H.Point) := by
      simp only [Finset.mem_insert, Finset.mem_singleton]
      push_neg
      exact ⟨hEqRa1, hEqRa, hEqιP1⟩
    simp only [if_neg hnmemS, if_neg hEqRa1, if_neg hEqRa, if_neg hEqιP1]
    ring

/-- **`f`'s `ordAt` at a root of the cross4 residual factor
`uCANewCross4`, expressed via `uCANewCross4`'s own `rootMultiplicity` —
...
cross4 objects substituted throughout. -/
theorem ordAt_eq_rootMultiplicity_of_uCANewCross4_root
    (hchar : (2 : k) ≠ 0) (hsf : Squarefree H.f)
    (Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 : k)
    (hdet : (caCross4InterpMatrix Ra1X RaX P1X).det ≠ 0)
    (h1 : Ra1X ≠ RaX) (h2 : Ra1X ≠ P1X) (h3 : RaX ≠ P1X)
    ...
    (hAeval : (denomPolyCACross4 Ra1X RaX P1X : k[X]).eval P.X ≠ 0)
    (hPY : P.Y = (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).eval P.X)
    (hPY_ne : P.Y ≠ 0)
    (m : ℕ)
    (hUmult : Polynomial.rootMultiplicity P.X
      (uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) = m) :
    ordAt P (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 : k[X]) (1 : k[X]) = (m : ℤ) := by
  have hne : H.f - (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) ^ 2 ≠ 0 := by
    intro hcontra
    apply hU_ne0
    show uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 = 0
    unfold uCANewCross4
    rw [hcontra, Polynomial.zero_divByMonic]
  set E := -bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 with hE_def
  set U := uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 with hU_def
  set A := denomPolyCACross4 Ra1X RaX P1X with hA_def
  have hAUraw : pairNorm H E (1 : k[X]) = A * (-U) := by
    unfold pairNorm
    have hfact := pairNormBCACross4_eq_denomPolyCACross4_mul_uCANewCross4 H
      Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hdet h1 h2 h3
      hRa1_curve hRa_curve hP1_curve hP1Deriv hne
    have hring : (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) ^ 2 - (1 : k[X]) ^ 2 * H.f =
        -(H.f - (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) ^ 2) := by ring
  ...
  have hgbar_ne_eval : E.eval P.X + (-(1 : k[X])).eval P.X * P.Y ≠ 0 := by
    ...
    rw [hPY]
    intro hcontra
    have h2 : (2 : k) * (bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2).eval P.X = 0 := by
...
/-- **`f`'s divisor restricted to the five named points, cross4 case:
`[Ra1] + 2•[Ra] + [ιP1] + [T1] + [T2]`.** Widens
`divToPair_eq_C_add_iotaA_of_split_cross4`'s three-point conclusion by
...
zero set; `T1X, T2X` are genuine additional roots of `uCANewCross4`). -/
theorem divToPair_eq_C_add_iotaA_add_T_of_split_cross4
    [DecidableEq H.Point]
    (hchar : (2 : k) ≠ 0) (hsf : Squarefree H.f)
    (Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 : k)
    (hdet : (caCross4InterpMatrix Ra1X RaX P1X).det ≠ 0)
    ...
    divToPair (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X])
        ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point) =
      single PtRa1 + (2 : ℤ) • single PtRa + single PtιP1 + single PtT1 + single PtT2 := by
  have h_bot : ∀ P : H.Point, pointIdeal P ≠ ⊥ := pointIdeal_ne_bot
  have h3pt : divToPair (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X])
      ({PtRa1, PtRa, PtιP1} : Finset H.Point) =
      single PtRa1 + (2 : ℤ) • single PtRa + single PtιP1 :=
    divToPair_eq_C_add_iotaA_of_split_cross4 hchar hsf Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2
      hdet h1 h2 h3 hRa1_curve hRa_curve hP1_curve hP1Deriv hRa1Y_ne hRaY_ne hP1Y_ne
      PtRa1 PtRa PtιP1 hPtRa1X hPtRa1Y hPtRaX hPtRaY hPtιP1X hPtιP1Y hne
      hU_evalRa1 hU_evalRa hU_evalP1
  have hne_of_X : ∀ {Q1' Q2' : H.Point}, Q1'.X ≠ Q2'.X → Q1' ≠ Q2' :=
    fun hX heq => hX (heq ▸ rfl)
  have hRa1Ra' : PtRa1 ≠ PtRa := hne_of_X (hPtRa1X ▸ hPtRaX ▸ h1)
  have hRa1ιP1' : PtRa1 ≠ PtιP1 := hne_of_X (hPtRa1X ▸ hPtιP1X ▸ h2)
  ...
  have hOrdRa1 : ordAt PtRa1 (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 1 := by
    ...
    have hMem : PtRa1 ∈ ({PtRa1, PtRa, PtιP1} : Finset H.Point) := by simp
    rw [if_pos hMem] at hL
    rw [hL]
    simp only [map_add, map_zsmul, coeffAt_single, smul_eq_mul, eq_self, ite_true]
    rw [if_neg hRa1Ra', if_neg hRa1ιP1']
    ring
  have hOrdRa : ordAt PtRa (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 2 := by
    have hL := congrArg (coeffAt PtRa) h3pt
    rw [coeffAt_divToPair] at hL
    have hMem : PtRa ∈ ({PtRa1, PtRa, PtιP1} : Finset H.Point) := by simp
    rw [if_pos hMem] at hL
    rw [hL]
    simp only [map_add, map_zsmul, coeffAt_single, smul_eq_mul, eq_self, ite_true]
    rw [if_neg (Ne.symm hRa1Ra'), if_neg hRaιP1']
    ring
  have hOrdιP1 : ordAt PtιP1 (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 1 := by
    have hL := congrArg (coeffAt PtιP1) h3pt
    rw [coeffAt_divToPair] at hL
    have hMem : PtιP1 ∈ ({PtRa1, PtRa, PtιP1} : Finset H.Point) := by simp
    rw [if_pos hMem] at hL
    rw [hL]
    simp only [map_add, map_zsmul, coeffAt_single, smul_eq_mul, eq_self, ite_true]
    rw [if_neg (Ne.symm hRa1ιP1'), if_neg (Ne.symm hRaιP1')]
    ring
  -- The two new residual-point `ordAt = 1` facts.
  have hmult1 : Polynomial.rootMultiplicity T1X
      (uCANewCross4 H Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) = 1 :=
  ...
  have hOrdT1 : ordAt PtT1 (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 1 := by
    have := ordAt_eq_rootMultiplicity_of_uCANewCross4_root hchar hsf
      Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hdet h1 h2 h3
      hRa1_curve hRa_curve hP1_curve hP1Deriv PtT1 (h_bot PtT1) hU_ne0
      hAeval1 hPtT1Y hPtT1Y_ne 1 (hPtT1X ▸ hmult1)
    simpa using this
  have hOrdT2 : ordAt PtT2 (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X]) = 1 := by
    have := ordAt_eq_rootMultiplicity_of_uCANewCross4_root hchar hsf
      Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hdet h1 h2 h3
      hRa1_curve hRa_curve hP1_curve hP1Deriv PtT2 (h_bot PtT2) hU_ne0
  ...
  by_cases hEqRa1 : P = PtRa1
  · rw [hEqRa1]
    have hMem : PtRa1 ∈ ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point) := by simp
    rw [if_pos hMem, hOrdRa1, if_pos rfl, if_neg hRa1Ra', if_neg hRa1ιP1',
      if_neg hRa1T1', if_neg hRa1T2']
    ring
  by_cases hEqRa : P = PtRa
  · rw [hEqRa]
    have hMem : PtRa ∈ ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point) := by simp
    rw [if_pos hMem, hOrdRa, if_neg (Ne.symm hRa1Ra'), if_pos rfl, if_neg hRaιP1',
      if_neg hRaT1', if_neg hRaT2']
    ring
  by_cases hEqιP1 : P = PtιP1
  · rw [hEqιP1]
    have hMem : PtιP1 ∈ ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point) := by simp
    rw [if_pos hMem, hOrdιP1, if_neg (Ne.symm hRa1ιP1'), if_neg (Ne.symm hRaιP1'), if_pos rfl,
      if_neg hιP1T1', if_neg hιP1T2']
    ring
  by_cases hEqT1 : P = PtT1
  · rw [hEqT1]
    have hMem : PtT1 ∈ ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point) := by simp
  ...
  · rw [hEqT2]
    have hMem : PtT2 ∈ ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point) := by simp
    rw [if_pos hMem, hOrdT2, if_neg (Ne.symm hRa1T2'), if_neg (Ne.symm hRaT2'),
      if_neg (Ne.symm hιP1T2'), if_neg (Ne.symm hT1T2'), if_pos rfl]
    ring
  · have hnmemS : P ∉ ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point) := by
      simp only [Finset.mem_insert, Finset.mem_singleton]
      push_neg
      exact ⟨hEqRa1, hEqRa, hEqιP1, hEqT1, hEqT2⟩
    rw [if_neg hnmemS, if_neg hEqRa1, if_neg hEqRa, if_neg hEqιP1, if_neg hEqT1, if_neg hEqT2]
    ring

/-- **`G₁`, cross4 case: `[Ra1] + 2•[Ra] + [ιP1] + [T1] + [T2] -
[T1]-[T2]-[ιT1]-[ιT2]-[δ₀]-[ιδ₀] ∈ principalSubgroup`.** Cross4 sibling
of `cIotaAmIotaT_mem_principalSubgroup`/`cIotaAmIotaT_mem_
principalSubgroup_tangent`, on the five-point `f`-support
`{PtRa1,PtRa,PtιP1,PtT1,PtT2}`. Same `divToPairRatio`/
`AddSubgroup.subset_closure` assembly as both predecessors,
`bCACross4_ordInfOfPair` supplying the `-6` pole order. -/
theorem cIotaAmIotaT_mem_principalSubgroup_cross4
  ...
  have hgoal_eq :
      (divToPair (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X])
        ...
        divToPair ((linX T1X * linX T2X) * linX δ₀.X) (0 : k[X])
          ({PtT1, PtT2, Point.iota PtT1, Point.iota PtT2, δ₀, Point.iota δ₀} : Finset H.Point)) =
      divToPairRatio (-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2) (1 : k[X])
          ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point)
        ((linX T1X * linX T2X) * linX δ₀.X) (0 : k[X])
          ({PtT1, PtT2, Point.iota PtT1, Point.iota PtT2, δ₀, Point.iota δ₀} : Finset H.Point) := rfl
  rw [hgoal_eq]
  apply AddSubgroup.subset_closure
  refine ⟨-bCACross4 Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2, 1,
    ({PtRa1, PtRa, PtιP1, PtT1, PtT2} : Finset H.Point),
  ...
  · rw [bCACross4_ordInfOfPair Ra1X RaX P1X Ra1Y RaY P1Y vDerivAtP2 hlead, ordInfOfPair_hT]
