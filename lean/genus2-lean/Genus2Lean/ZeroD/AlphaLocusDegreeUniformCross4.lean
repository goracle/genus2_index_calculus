...
/-! # Cross-pair tangent (`Ra2 = ι(sa.P2)`) sibling of
`reducedClass_eq_of_isReduction'`

`ROADMAP-cawitness-tangent-interpolation.md`, Part B, case 3, step 5's
"still open" item: the top-level theorem consuming `cAmιTmδmιδ_mem_of_le_cross4`
...
`Ra` and doubled with `sa.P2`, with `Ra2` surviving as an ordinary anchor
point. Here it is the other way round: `Ra1` survives as an ordinary anchor
point, and `Ra2` is renamed `Ra` and doubled with `sa.P2` via
`hP2eq : sa.P2 = Point.iota Ra`. The anchor pair `{Ra1, Ra}` stays genuinely
split (`caCross4InterpMatrix`'s own `h1 : Ra1X ≠ RaX` — checked directly,
`CAWitnessCrossTangentV4.lean`); `Sanchor`'s own construction
...
2. `caInterpMatrix`/`caCoeff`/`bCA`/`uCANew`/`denomPolyCA` (four free
   points `Ra1,Ra2,P2,P1`) are replaced by `caCross4InterpMatrix`/
   `caCross4Coeff`/`bCACross4`/`uCANewCross4`/`denomPolyCACross4` (three
   free points `Ra1X,RaX,P1X` plus the derivative datum `vDerivAtP2` —
   `CAWitnessCrossTangentV4.lean`), matching `cAmιTmδmιδ_mem_of_le_cross4`'s
   own signature exactly.
3. `h1P2,h1P1,h2P2,h2P1,hPP` (six-hypothesis-family's cross/target members)
   disappear entirely — cross4's construction only needs `h1,h2,h3`
   (`Ra1X≠RaX`, `Ra1X≠P1X`, `RaX≠P1X`), the three-hypothesis nondegeneracy
...
/-- **Cross variant 4 top-level wiring**: `sa.reducedClass + q = toJacobian D
(target)`, for the case `sa.P2 = ι(Ra2)` (`Ra2`'s own conjugate collides
with the target's first point). Signature is `reducedClass_eq_of_isReduction'`'s
own, with `Ra2` renamed `Ra`, `sa.P2` forced via `hP2eq`, the six-hypothesis
...
theorem reducedClass_eq_of_isReduction'_cross4 {p : ℕ} [Fact (Nat.Prime p)] [Fact (p ≠ 2)]
    ...
    -- `caCross4InterpMatrix` construction's own convention
    -- (`CAWitnessCrossTangentV4.lean`, `Ra2 = ι(sa.P2)`).
    (Ra1 Ra : H.Point)
    (hRa12Xne : Ra1.X ≠ Ra.X)
    ...
    -- **The cross-pair `CAWitness` identification**: `caCross4InterpMatrix`'s
    -- own three free points `Ra1X,RaX,P1X` (`sa.P2` is gone, replaced by
    -- the `ι(Ra)`-identification `hP2eq` above), plus the target-derivative
    -- datum `vDerivAtP2` (`= -(derivative v).eval sa.P2.X`, per
    -- `CAWitnessCrossTangentV4.lean`'s own `hP1Deriv` convention).
    (vDerivAtP2 : F p)
    (hdet : (caCross4InterpMatrix Ra1.X Ra.X sa.P1.X).det ≠ 0)
    (hlead : caCross4Coeff Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2 3 ≠ 0)
    (h1 : Ra1.X ≠ Ra.X) (h2 : Ra1.X ≠ sa.P1.X) (h3 : Ra.X ≠ sa.P1.X)
    (hRa1_curve : Ra1.Y ^ 2 = H.f.eval Ra1.X) (hRa_curve : Ra.Y ^ 2 = H.f.eval Ra.X)
    (hP1_curve : sa.P1.Y ^ 2 = H.f.eval sa.P1.X)
    (hP1Deriv : 2 * Ra.Y * (-vDerivAtP2) = (derivative H.f).eval Ra.X)
    (hP1Y_ne : sa.P1.Y ≠ 0)
    (hne : H.f - (bCACross4 Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2) ^ 2 ≠ 0)
    (hU_evalRa1 : (uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2).eval Ra1.X ≠ 0)
    (hU_evalRa : (uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2).eval Ra.X ≠ 0)
    (hU_evalP1 : (uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2).eval sa.P1.X ≠ 0)
    (hU_ne0 : uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2 ≠ 0)
    (PtT1 PtT2 : H.Point)
    (hPtT1X : PtT1.X ≠ PtT2.X)
    (hPtT1 : (uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2).IsRoot PtT1.X)
    (hPtT2 : (uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2).IsRoot PtT2.X)
    (Q1 Q2 : Polynomial (F p))
    (hQ1_def : uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2 =
      (Polynomial.X - Polynomial.C PtT1.X) * (Polynomial.X - Polynomial.C PtT2.X) * Q1)
    (hQ1T1 : Q1.eval PtT1.X ≠ 0)
    (hQ2_def : uCANewCross4 H Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2 =
      (Polynomial.X - Polynomial.C PtT2.X) * (Polynomial.X - Polynomial.C PtT1.X) * Q2)
    (hQ2T2 : Q2.eval PtT2.X ≠ 0)
    (hAeval1 : (denomPolyCACross4 Ra1.X Ra.X sa.P1.X : Polynomial (F p)).eval PtT1.X ≠ 0)
    (hAeval2 : (denomPolyCACross4 Ra1.X Ra.X sa.P1.X : Polynomial (F p)).eval PtT2.X ≠ 0)
    (hPtT1Y : PtT1.Y = (bCACross4 Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2).eval PtT1.X)
    (hPtT1Y_ne : PtT1.Y ≠ 0)
    (hPtT2Y : PtT2.Y = (bCACross4 Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2).eval PtT2.X)
    (hPtT2Y_ne : PtT2.Y ≠ 0)
    ...
    (hsupp_f : ∀ P, P ∉ ({Ra1, Ra, Point.iota sa.P1, PtT1, PtT2} :
        Finset H.Point) →
      ordAt P (-bCACross4 Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2) (1 : Polynomial (F p)) = 0)
    (hspec_f : ∀ vv : IsDedekindDomain.HeightOneSpectrum (CoordinateRing H),
      (Associates.mk vv.asIdeal).count (Associates.mk (Ideal.span
        ({toPair H (-bCACross4 Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2) 1} :
          Set (CoordinateRing H)))).factors ≠ 0 → ∃ P, vv.asIdeal = pointIdeal P)
    [∀ P : ({Ra1, Ra, Point.iota sa.P1, PtT1, PtT2} : Finset H.Point),
      Module.Finite (F p) (CoordinateRing H ⧸ pointIdeal P.1 ^
        (ordAt P.1 (-bCACross4 Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2)
  ...
  -- **The cross4 concrete-coordinate assembly (†)**: `[Ra1]+[Ra]-[ιRa] -
  -- [ιT1]-[ιT2] - [δ₀]-[ιδ₀]`-shaped fact, then rewritten below (via
  -- `hcoe`) into `[Ra1]+[Ra]-[ι(Ra)]-[sa.P1]-...` matching this theorem's
  -- own `single Ra1 + single Ra2 - single sa.P2 - single sa.P1 - ...`
  -- shape once `hP2eq` identifies `single sa.P2 = single (Point.iota Ra)`.
  have hDP := cAmιTmδmιδ_mem_of_le_cross4 (H := H) hdeg hchar hsf D hD
    hspec_linX
    Ra1.X Ra.X sa.P1.X Ra1.Y Ra.Y sa.P1.Y vDerivAtP2
    hdet hlead h1 h2 h3
    hRa1_curve hRa_curve hP1_curve hP1Deriv hRa1Y_ne hRaY_ne hP1Y_ne
    Ra1 Ra sa.P1 (Point.iota sa.P1)
    rfl rfl rfl rfl rfl rfl rfl rfl
    hne
    hU_evalRa1 hU_evalRa hU_evalP1 hU_ne0
  ...
  have hcoe : ((aAnchor - aP2P1Nι : Divisor0 H) : Divisor H) -
      ...
      (single Ra1 + single Ra - single sa.P2 - single sa.P1 -
