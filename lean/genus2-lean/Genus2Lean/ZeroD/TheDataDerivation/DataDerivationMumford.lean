...
mentions; it says nothing about whether `(towerToRdec sg v).1 / (towerToRdec
sg v).2` (as a fraction) actually equals the field element `v` it is
supposed to represent. Nothing anywhere else in this file or in
`DecoupledSystemRegular.lean` establishes that connection either.
`towerToRdec_spec_TODO : True := trivial` (the previous placeholder at this
...
image inside the concrete function field generated by `sg.tGen`/`sg.wGen`,
subject to the two curve relations `wGen i ^ 2 = f (tGen i)` — i.e. `ι`
sends the abstract tower generators `t1,t2,w1,w2` to
`X (sg.tGen 0), X (sg.tGen 1), X (sg.wGen 0), X (sg.wGen 1)` respectively
(as elements of `FractionRing (MvPolynomial Vars (F p))`), extended
multiplicatively/additively. No such `ι` is constructed anywhere in this
project; until it is, this theorem cannot even be instantiated on a
concrete embedding, only carried as a hypothesis-shaped statement. Both the
