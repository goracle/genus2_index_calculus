...
were `sorry` as of this pass, downstream of `DataDerivationSolve.lean`'s
`dvd_N_u` in particular. **Status, later pass**: this whole file is now
`sorry`-free — `dvd_N_u` has since been fully proved in
`DataDerivationSolve.lean`, and `vRS_sq_eq_f_mod_uRS` below is proved
outright as a theorem taking `hcur`/`hNu`/`hInv` as explicit hypotheses
(the divisibility facts, once proved elsewhere, are threaded in rather
than re-derived here) — see that theorem's own docstring and the
`BridgeToRdec` section below, which is likewise fully constructed
(`towerToRdecK1`, `baseFracToRing`, etc.), not the `sorry`-stub the
"New item" note further down in this docstring originally described.
...
**Was left as `sorry`** throughout this section as of the pass that wrote
this note: a full port needed (a) a concrete ring isomorphism/embedding
identifying `K2`'s rank-4-over-`K0` structure with the sub-`F`-algebra of
`Rdec` generated by `{wa1,wa2,a1,a2}` (resp. `{wb1,wb2,b1,b2}` for the
b-side copy) subject to the two curve relations — i.e. `K2 ≃ Rdec ⧸
(curve relations)`'s fraction field, restricted to the 4-generator
subring — and (b) clearing each coefficient's denominator down to a
genuine `Rdec` numerator/denominator pair (`tower_to_ring`'s own
per-coefficient `_reduce_frac`, itself skipped per the note above, but
the denominator-clearing ITSELF, as opposed to GCD-reducing it
afterward, is not skippable — `Rdec` has no fractions at all, so SOME
clearing step is mandatory even though further reduction to lowest terms
is not). **Status, later pass**: both (a) and (b) are now built below
(`baseFracToRing`, `towerToRdecK1`, and their `K2`-level counterparts
further down this file) — see each definition's own "No `sorry`"-tagged
docstring for the construction actually used; nothing in this section is
a bare `sorry` any longer. -/
...
/-- **Correctness spec for `towerToRdec`, proved (see `towerToRdec_spec`
below).** `towerToRdec_vars_subset` above only bounds *which variables* the
output mentions; `towerToRdec_spec` (further down this file) is the
connection that was previously missing — that `(towerToRdec sg v).1 /
(towerToRdec sg v).2` (as a fraction) actually equals the field element `v`
it is supposed to represent. A now-superseded placeholder,
`towerToRdec_spec_TODO : True := trivial`, once stood in for this fact
without asserting anything; `towerToRdec_spec` is the real statement, with a
real proof.

It is stated against an explicit embedding `ι` taken as a hypothesis:
`ι : K2 p c0 c1 c2 c3 c4 →+* FractionRing (MvPolynomial Vars (F p))` is the
map identifying `K2`'s abstract tower element with its image inside the
concrete function field generated by `sg.tGen`/`sg.wGen`, subject to the two
curve relations `wGen i ^ 2 = f (tGen i)` — i.e. `ι` sends the abstract
tower generators `t1,t2,w1,w2` to `X (sg.tGen 0), X (sg.tGen 1),
X (sg.wGen 0), X (sg.wGen 1)` respectively (as elements of `FractionRing
(MvPolynomial Vars (F p))`), extended multiplicatively/additively. No
concrete construction of `ι` exists yet in this project, so the theorem is
proved for an arbitrary `ι` satisfying `hι_t`/`hι_w1`/`hι_w2` rather than
instantiated on one — that construction remains separate, unattempted work
(part of `Reduce`, per `ZeroD-README.md`'s dependency chain), but does not
block this theorem's own proof, which is complete. -/
...
set_option maxHeartbeats 4000000 in
/-- **Extracted, one level down: `towerToRdecK1`'s own `IsRdecWitness`
spec.** This is `towerToRdec_spec`'s proof's own internal `hK1spec`
`have`, promoted to a standalone theorem — same content, not new math,
just no longer buried inside the bigger proof where it couldn't be
reused. States exactly the `K1`-level analogue of `towerToRdec_spec`:
for `x : K1`, `towerToRdecK1 p sg x` is an honest numerator/denominator
witness for `x` itself (as an element of `K2`, via the same ambient
`ι : K2 →+* L` `towerToRdec_spec` takes — deliberately not `K1 →+* L`,
matching how the original proof actually invokes it, composed against
`algebraMap K1 K2`). Needs only `hι_t`/`hι_w1` (no `hι_w2`): `K1`'s own
tower level never touches `w2`. **Why this matters**: this is exactly
the fact `TowerCoeffWitnessDescent.lean`'s `coeffDescent_totalDegree_le`
needs at the call site — `towerToRdec p sg v`'s definition destructures
into `towerToRdecK1 p sg d0`/`towerToRdecK1 p sg d1` (`d0,d1` the
`K2_poly`-normal-form coefficients of `v`), and this theorem says
those two calls are themselves valid `IsRdecWitness` pairs for `d0,d1`
respectively — i.e. `towerToRdec`'s own `(n0,den0)`/`(n1,den1)`
intermediate pairs ARE the `(N0,D0)`/`(N1,D1)` witness pairs that
lemma's hypotheses ask for, not merely opaque intermediate `let`
bindings. Proof is a verbatim copy of the original `hK1spec` block,
unindented. **Heartbeats raised as a precaution**: the original
`towerToRdec_spec` needed this for its combined `unfold towerToRdecK1`/
`unfold towerToRdec` steps; this proof only does the lighter
`unfold towerToRdecK1` half, so may not need the raised limit, but
matching the neighboring theorem's guard rather than assuming it's
unnecessary. -/
theorem towerToRdecK1_spec {Vars : Type*}
    (sg : SideGens Vars) (x : K1 p c0 c1 c2 c3 c4)
    ...
    (algebraMap (MvPolynomial Vars (F p)) (FractionRing (MvPolynomial Vars (F p)))
        (towerToRdecK1 p sg x).1) =
      (algebraMap (MvPolynomial Vars (F p)) (FractionRing (MvPolynomial Vars (F p)))
        (towerToRdecK1 p sg x).2) *
  ...
  have hbase : ∀ w : K0 p,
    ...
    exact hmulι
  have hK1repr (y : K1 p c0 c1 c2 c3 c4) :
      y = algebraMap (K0 p) (K1 p c0 c1 c2 c3 c4)
          ((AdjoinRoot.modByMonicHom (K1_poly_monic p c0 c1 c2 c3 c4) y).coeff 0) +
        algebraMap (K0 p) (K1 p c0 c1 c2 c3 c4)
          ((AdjoinRoot.modByMonicHom (K1_poly_monic p c0 c1 c2 c3 c4) y).coeff 1) *
            w1 p c0 c1 c2 c3 c4 := by
    exact adjoinRoot_quadratic_normal_form
      (K1_poly_monic p c0 c1 c2 c3 c4) (natDegree_X_pow_sub_C) y
  unfold towerToRdecK1
  dsimp only
