...
theorem reducedClass_eq_of_isReduction'_tangent {p : ℕ} [Fact (Nat.Prime p)] [Fact (p ≠ 2)]
    ...
    -- **PROBE A: CAWitness/tangent-specific hypotheses (hdet..hQ2T2)
    -- stubbed to trivial placeholder types for isolation testing.**
    -- If this build succeeds (no whnf timeout), the cost is in THIS
    -- zone (caTangentInterpMatrix/caTangentCoeff/bCATangent/uCA-heavy
    -- terms). If it still times out, the cost is elsewhere.
    ...
    -- **PROBE B: additionally stub the bCATangent/denomPolyCATangent
    -- tail too** — full isolation of the entire CAWitness-tangent zone.
    (hAeval1 : True)
    (hAeval2 : True)
    (PtT1Y_val PtT2Y_val : F p)
    (hPtT1Y : PtT1.Y = PtT1Y_val)
    (hPtT1Y_ne : PtT1.Y ≠ 0)
    (hPtT2Y : PtT2.Y = PtT2Y_val)
    (hPtT2Y_ne : PtT2.Y ≠ 0)
