...
theorem isSMulRegular_den_of_second_peel
  ...
  classical
  intro τ Fu0' Fu1' d' hFu0' hFu1' hd'
  ...
  have hf_inj : Function.Injective (fun v : τ' => (v.1.1 : Idx)) := by
    ...
    exact hvw
  have hu1num0_range : (↑u1num0.vars : Set Idx) ⊆ Set.range (fun v : τ' => (v.1.1 : Idx)) := by
    intro v hv
  ...
  have hu1den0_range : (↑u1den0.vars : Set Idx) ⊆ Set.range (fun v : τ' => (v.1.1 : Idx)) := by
    intro v hv
    have hmem := hu1_indep0 v (by
    ...
    have hne0 : v ≠ U0 := by rintro rfl; fin_cases hmem
    exact ⟨⟨⟨v, hne1⟩, by rw [hx0_def]; exact fun h => hne0 (congrArg Subtype.val h)⟩, rfl⟩
  have hu2num0_range : (↑u2num0.vars : Set Idx) ⊆ Set.range (fun v : τ' => (v.1.1 : Idx)) := by
    intro v hv
    ...
    have hne0 : v ≠ U0 := by rintro rfl; fin_cases hmem
    exact ⟨⟨⟨v, hne1⟩, by rw [hx0_def]; exact fun h => hne0 (congrArg Subtype.val h)⟩, rfl⟩
  have hu2den0_range : (↑u2den0.vars : Set Idx) ⊆ Set.range (fun v : τ' => (v.1.1 : Idx)) := by
    intro v hv
    have hmem := hu2_indep0 v (by
    ...
    have hne0 : v ≠ U0 := by rintro rfl; fin_cases hmem
    exact ⟨⟨⟨v, hne1⟩, by rw [hx0_def]; exact fun h => hne0 (congrArg Subtype.val h)⟩, rfl⟩
  have hu1den1_range : (↑u1den1.vars : Set Idx) ⊆ Set.range (fun v : τ' => (v.1.1 : Idx)) := by
    intro v hv
  ...
  have hcomp : (fun v : τ' => (v.1.1 : Idx)) =
      (Subtype.val : τ → Idx) ∘ (Subtype.val : τ' → τ) := rfl
  ...
  have hFu0'_reg_opt : IsSMulRegular
    ...
    · have hbot := (isSMulRegular_bot_module_iff
          (R := MvPolynomial (Option τ') (F p))
          (MvPolynomial.rename (some : τ' → Option τ') d0'')).mp hd0''_domreg
      rw [List.map_nil]
      rw [show Ideal.ofList ([] : List (MvPolynomial (Option τ') (F p))) =
          (⊥ : Ideal (MvPolynomial (Option τ') (F p))) from Ideal.ofList_nil]
      exact hbot
    · rfl
  ...
  have hFu0'_reg : IsSMulRegular
    ...
    have hbridge :
        IsSMulRegular
          (MvPolynomial τ (F p) ⧸
            Ideal.ofList (([] : List (MvPolynomial τ' (F p))).map (fun q : MvPolynomial τ' (F p) =>
          ...
          (Ideal.Quotient.mk
            ...
            ((MvPolynomial.renameEquiv (F p) (optionSplit x0)).symm
              (MvPolynomial.rename some (c0'' - MvPolynomial.X x0 * d0''))))
        ↔
        IsSMulRegular
          (MvPolynomial (Option τ') (F p) ⧸
            Ideal.ofList (([] : List (MvPolynomial τ' (F p))).map (MvPolynomial.rename some)))
          (Ideal.Quotient.mk
            (Ideal.ofList (([] : List (MvPolynomial τ' (F p))).map (MvPolynomial.rename some)))
            (MvPolynomial.rename some (c0'' - MvPolynomial.X x0 * d0''))) :=
      isSMulRegular_bridge_prefix_gen (σ := τ) (F p) x0
    ...
    simp only [List.map_nil, hcollapse] at hbridge
    have hg_eq : MvPolynomial.rename (Subtype.val : τ' → τ) (c0'' - MvPolynomial.X x0 * d0'') = Fu0' := by
      rw [hFu0'_eq, map_sub, map_mul, MvPolynomial.rename_X]
    rw [hg_eq] at hbridge
    rw [hbridge]
    have hmap : MvPolynomial.rename (some : τ' → Option τ') (c0'' - MvPolynomial.X x0 * d0'') =
        MvPolynomial.rename some c0'' - MvPolynomial.X none * MvPolynomial.rename some d0'' := by
  ...
  have hc0''_predA : (↑c0''.vars : Set τ') ⊆ {v : τ' | predA v} := by
    ...
    have hmem : ((fun w : τ' => (w.1.1 : Idx)) v) ∈ ({wa1, wa2, a1, a2} : Finset Idx) := by
      have hrange : (v.1.1 : Idx) ∈ (↑u1num0.vars : Set Idx) := by
        ...
        have hdeg' : MvPolynomial.degreeOf ((fun v : τ' => (v.1.1 : Idx)) v)
            (MvPolynomial.rename (fun v : τ' => (v.1.1 : Idx)) c0'') ≠ 0 := by
          rw [hdeg_rename]
          exact hdeg
        rw [hc0''] at hdeg'
  ...
  have hd1''_predA : (↑d1''.vars : Set τ') ⊆ {v : τ' | ¬ predA v} := by
    ...
    have hmem : ((fun w : τ' => (w.1.1 : Idx)) v) ∈ ({wb1, wb2, b1, b2} : Finset Idx) := by
      have hrange : (v.1.1 : Idx) ∈ (↑u2den0.vars : Set Idx) := by
        ...
        have hdeg' : MvPolynomial.degreeOf ((fun v : τ' => (v.1.1 : Idx)) v)
            (MvPolynomial.rename (fun v : τ' => (v.1.1 : Idx)) d1'') ≠ 0 := by
          rw [hdeg_rename]
          exact hdeg
        rw [hd1''] at hdeg'
        exact (MvPolynomial.mem_vars_iff_degreeOf_ne_zero).mpr hdeg'
  ...
  have hFu1'_reg : IsSMulRegular
    ...
    have hbridge :
        IsSMulRegular
          (MvPolynomial τ (F p) ⧸
            Ideal.ofList (([c0''] : List (MvPolynomial τ' (F p))).map (fun q : MvPolynomial τ' (F p) =>
              ((MvPolynomial.renameEquiv (F p) (optionSplit x0)).symm (MvPolynomial.rename some q) :
                MvPolynomial τ (F p)))))
          (Ideal.Quotient.mk
            (Ideal.ofList (([c0''] : List (MvPolynomial τ' (F p))).map (fun q : MvPolynomial τ' (F p) =>
              ((MvPolynomial.renameEquiv (F p) (optionSplit x0)).symm (MvPolynomial.rename some q) :
                MvPolynomial τ (F p)))))
            ((MvPolynomial.renameEquiv (F p) (optionSplit x0)).symm
              (MvPolynomial.rename some (c1'' - MvPolynomial.X x0 * d1''))))
        ↔
        IsSMulRegular
          (MvPolynomial (Option τ') (F p) ⧸
            Ideal.ofList (([c0''] : List (MvPolynomial τ' (F p))).map (MvPolynomial.rename some)))
