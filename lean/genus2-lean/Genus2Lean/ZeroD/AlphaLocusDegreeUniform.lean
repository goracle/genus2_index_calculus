...
structure SampleTargetFromAlpha (p : ℕ) [Fact (Nat.Prime p)]
  ...
  **Update: `Reduce` now exists (`Reduce.AlphaReduce`/`Reduce.GeneralSharedRoot`,
  ...
  isReduction : Prop
  /-- **Standing hypothesis, per `ROADMAP-alpha-locus.md`'s "Newest status
  update" (`Stab(S)` argument for `Δ=0`): this sample's pair-sum class
  lies in the cyclic subgroup generated by `aClass`.**

  This is the CORE assumption the whole `Δ = 0` rigidity argument rests
  on, and it is deliberately assumed here, not derived — it is NOT
  provable from `H.Point`/`Jacobian H D` alone (that was the whole
  reason `OrbitMapConstant.lean`'s connectedness route was attempted and
  needed machinery this codebase doesn't have). It is true of any real
  DLP instance per advisory-7 §1's actual sampling procedure (points
  drawn via cofactor-clearing against `J`'s true order `h·ell`, landing
  them inside the fixed prime-order subgroup `⟨a⟩`), i.e. it is an
  honest fact about how the cryptographic instance is constructed, in
  exactly this project's usual honest-hypothesis style
  (`CrossNondegenerate`/`isReduction` above).

  Stated at the PAIR-SUM level (`s D δ₀ P1 + s D δ₀ P2`, not each point
  separately) because that is exactly the quantity
  `matching_solutions_translate_by_delta`/
  `MatchingSolutionSwapSymmetry.fixedTargetSolutions_ncard_le_four` build
  `Δ` out of (`Δ := (P1+P2) - (P1'+P2')` in that file's notation) — this
  shape lets the roadmap's step 3 ("`Δ ∈ ⟨a⟩` for free... a difference of
  two elements already assumed to lie in the subgroup, by subgroup
  closure") go through with a one-line `AddSubgroup.sub_mem` call, no
  separate per-point bundling needed. See
  `SampleTargetFromAlphaPairMemA.lean` for that composition, packaged
  against `StabOfSmallSetTrivial.lean`'s
  `delta_eq_zero_of_stabilizes_small_set` directly.

  **Do not weaken this to something more convenient** (e.g. "the
  difference of two samples' pair-sums lies in `⟨a⟩`," which would be
  true whenever EITHER assumption below is false, given the wrong
  cyclic group, and would silently break the `Stab(S)` argument's
  actual content) — the real goal is that the relations matrix itself
  lives in `⟨a⟩`, which forces every `P` sampled against it to live in
  `⟨a⟩` too; that per-point (here per-pair-sum) membership is the fact
  to preserve, not a derived consequence to reconstruct after the
  fact. -/
  memZmultiplesA :
