...
theorem reducedClass_eq_of_isReduction'_tangent {p : ℕ} [Fact (Nat.Prime p)] [Fact (p ≠ 2)]
    ...
    -- **PROBE D: keep hdet/hlead/hne real; stub the uCANewTangent-applied
    -- hypotheses (uCA opaque, no equation) instead.** Complementary to
    -- probe C — isolates whether the *applied* uCANewTangent occurrences
    -- (as opposed to the matrix-det/Cramer's-rule terms) are the driver.
    (uCA : Polynomial (F p))
