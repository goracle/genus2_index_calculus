...
This file previously continued with an abandoned route to `IsDedekindDomain (CoordinateRing H)`
via `IsIntegralClosure.isDedekindDomain` over an abstract `variable {L : Type*} [...]` block of
fraction-field hypotheses (§3.1 `coordinateRing_isIntegrallyClosed_in_fractionField`, §3.2
`coordinateRingIsDedekindDomain_viaIntegralClosure`), both left as `sorry`. That route is
superseded: `PrincipalDivisorsIntegralClosure.lean` now proves `coordinateRing_isIntegrallyClosed`
and `coordinateRingIsDedekindDomain` directly and completely (no `sorry`), via
`DedekindClosure5.mem_integralClosure_iff_poly_coefficients` applied to
`FractionRing (CoordinateRing H)` itself rather than an abstract `L`. Nothing outside this file
referenced the removed §3.1/§3.2 theorems (confirmed by grep across the project), so the section
was deleted rather than carrying two dead `sorry`s forward. `sq_mul_mem_of_squarefree` above is
still load-bearing (used by `DedekindClosure5.lean` and `PrincipalDivisorsIntegralClosure.lean`)
