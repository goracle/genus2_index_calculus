...
-- standing hypothesis (not a convenience): `uniqueDegree2MapToP1` and
-- `finrank_L_pair` are both false over a non-algebraically-closed `k`,
-- via the `z = 1/q(x)` counterexample documented there.
